...
- That have been annotated with cerERR_NETWORK_CHANGEDtain types of annotations (e.g. Localized
...
def download_one_image(bucket, split, image_id, download_folder):
  try:
    ...
    # modified
    bucket.download_file(f'{split}/{image_id}.jpg',
                         os.path.join(download_folder,split, f'{image_id}.jpg'))
...
def download_all_images(args):
  ...
  try:
    ...
    # Modified to handle re-run issues 
    new_image_list =[]
    for image in image_list :
      image_id = f"{image[1]}.jpg"
      if not os.path.exists(os.path.join(args['download_folder'],image_id)) :
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      '--download_folder',
      type=str,
      default=os.path.join('.','data'),    # original is None
